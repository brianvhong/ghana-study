...
For each primary outcome variables we would like to test whether it is differently affected by two interventions. An unadjusted and an adjusted linear model is fit to test the hypothesis. The unadjusted model contains only the *Treatment* group, while the adjusted model also contains covariate variables.
...
df_covariate = make_dummy_variables(df_covariate)
```
...
```{r}
models = ""
covar = lapply(vars_outcome, function(outcome){
    y = df_outcome[,outcome]
    # find covariate variables with p smaller than 0.1 in step 2
    covariates = colnames(df_covariate)[bivar$p[outcome,] < 0.1]
    ...
    # global_env = parent.env(environment())
...
fits = lapply(vars_outcome, function(outcome){
    ...
    # adjusted model
    covariates = colnames(df_covariate)[!is.na(covar[outcome,])]
    design = design %>%
        cbind(df_covariate[, covariates]) %>%
        as.matrix
    fit2 = lm(y ~ design)
    ci2 = confint(fit2)[2,]
    fit2 = summary(fit2)
    ...
    colnames(fit) = c("response", "model", "beta1", "stderr", "tval", "pval", "ci25", "ci975")
    return(fit)
}) %>%
    do.call(rbind, .) 
pretty_table2(fits)
...
```{r, echo = FALSE}
fits = lapply(c("adjusted", "unadjusted"), function(m){
    x = filter(fits, model == m)
    x = column_to_rownames(x, "response")
    x
}) %>%
    `names<-`(c("adjusted", "unadjusted"))
