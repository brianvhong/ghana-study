...
$$y = \beta_0 + \beta_1 x$$
* Adjusted model: 
$$y = \beta_0 + \beta_1 x + \gamma_1 z_1 + \gamma_2 z_2 + ... + \gamma_k z_k$$

$y$ is any outcome variable from the outcome matrix $Y$. $x$ is any predictor variable from the predictor matrix $X$. $\beta_0$ is the intercept, and $\beta_1$ is the slop. $z_1$ to $z_k$ are covariate variables selected, and $\gamma_1$ to $\gamma_k$ are the corresponding coefficients to covariate vairables.
...
Next, a complete model was fit between any **predictor variable** $x$ and **outcome variable** $y$, with any covariates $z$ that is significant in step 2.
...
Evantually, the adjusted model was built for any given **outcome variable** $y$ and **predictor variable** $x$. The covariate used in any adjusted model must have a p value < 0.1 in step 2, and a p value < 0.05 in step 3. 
